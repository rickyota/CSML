...
def get_listdir(dname):
    ...
    fnames = sorted(filter(lambda f: not f.startswith("."), fnames))
...
def check_names_same(dname_train, dname_label):
    """check image names in train and label are the same.

    Args:
        dname_train ([type]): [description]
        dname_label ([type]): [description]

    Returns:
        [type]: [description]
    """
    fnames_train = get_listdir(dname_train)
    fnames_label = get_listdir(dname_label)

    if len(fnames_train) != len(fnames_label):
        return False

    for f_t, f_l in zip(fnames_train, fnames_label):
        im_t = os.path.splitext(os.path.basename(f_t))[0]
        im_l = os.path.splitext(os.path.basename(f_l))[0]
        if im_t != im_l:
            return False
