...
class FCN(Chain):
	...
	def __init__(self):
		...
		super(FCN, self).__init__(
			conv1_1=L.Convolution2D(1, self.n_layer1, 3, stride=1, pad=1, initialW=initializer),
			conv1_2=L.Convolution2D(self.n_layer1, self.n_layer1, 3, stride=1, pad=1, initialW=initializer),
			conv2_1=L.Convolution2D(self.n_layer1, self.n_layer2, 3, stride=1, pad=1, initialW=initializer),
			conv2_2=L.Convolution2D(self.n_layer2, self.n_layer2, 3, stride=1, pad=1, initialW=initializer),
			convcha3=L.Convolution2D(self.n_layer2, 2, 1, stride=1, pad=0, initialW=initializer),
			deconvsize3=L.Deconvolution2D(2, 2, 4, stride=2, pad=1, initialW=initializer),
	...
	def im_stan(self, x_batch):
		...
		x_batch_mean = x_batch_mean.reshape([x_batch.shape[0], x_batch.shape[1], 1, 1])
		x_batch_std = x_batch_std.reshape([x_batch.shape[0], x_batch.shape[1], 1, 1])
